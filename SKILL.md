...
### HTML 리포트 특징
- **랜딩형 레이아웃**: 딥그린 히어로 패널 + 크림 배경 + 라임 액센트,
  상단 모듈 내비(예금/적금/금리 추이/상품 비교/내 조건 — 클릭 시 해당 섹션 연결).
- **LLM 채팅(히어로 입력창)**: 🔑 버튼에 Anthropic API 키를 넣으면 **Claude가 실제로
  응답**(claude-sonnet-4-6, 오늘자 전체 상품 데이터를 컨텍스트로 전달, 키는 localStorage에만
  저장). 키가 없거나 호출 실패 시 내장 규칙 기반 에이전트로 자동 폴백.
